...
// these loop assign all slots are available
...
sample TestCase1:
             Input                                      Output
     1.Club House,2012-10-26,16:00,22:00             Booked,Rs.1000  
 Own Assumption;
         we write a program based on your sample output(eg:16:00,22:00)
  (for my assumptions based on your output ,i will taken only two hours 2*500=1000)

positive testing:
        If User enter the Valid time then Book the slot and calculate the amount based on time slot.
          Case1:
              If user enter the time1 and time2 inbetween 10 to 16 then book the slot and add the amount based on time slot 100 per hour then calculate it.
          Case 2:     
            If user enter the time1 and time2 inbetween 16 to 22 then book the slot and add the amount based on time slot 500 per hour then calculate it.
         Case 3:
         If user enter the time1 and time2 either 10 to 16 or 16 to 22 then book the slot and calculate the amount.
Negative testing:
           Suppose user enter the unvalid time(eg:0<=t1,t2<10,23<=t1,t2<=24).

sample TestCase2:

             Input                                      Output
 2.Tennis Court,2012-10-26,10:00,20:00            Booked,Rs.500

 Own Assumption;

 we write a program based on your sample output(eg:10:00,20:00)
  (for my assumptions based on your output ,i will taken range of hours ,because our output is 500 it  means 10 to 20 ,Calculate amount based on time slot 50 per hour 10*50=500 )

Positive Testing:
 If User enter the time slot duration is available then only Book the slot and calculate the amount based on time slot.
Negative testing:
           Suppose user enter the time slot duration is not available ,booking failed already booked ,try again.
sample TestCase3:

  Input                                             Output
     3.Club House,2012-10-26,16:00,22:00         Booking failed already booked 
